...
- NEW: Adds a health check to the main loop to prevent stale process state.
"""
import os
import sys
import signal
...
if __name__ == "__main__":
    ...
    try:
        # Initial call to on_doc_snapshot with current state (read before attaching listener)
        # This ensures initial state is handled before the loop starts.
        # Although on_snapshot typically handles this, an explicit initial fetch can ensure state sync
        
        # Read initial state explicitly to initialize global state variables correctly
        initial_stream_doc = stream_ref.get()
        initial_settings_doc = settings_ref.get()
        on_doc_snapshot([initial_stream_doc, initial_settings_doc], [], None) # Pass a list of snapshots
        ...
        while True:
            ...
            # If the process died but the state is still TRUE, reset the state.
            if camera_server_process and camera_server_process.poll() is not None:
                logger.error(f"[HEALTH CHECK] Camera Server (PID {camera_server_process.pid}) died unexpectedly. Resetting state.")
                stop_camera_server() # This sets current_streaming_enabled_state to False
                
                # Check if motion capture was waiting to be restarted and if settings allow it
                settings_data = settings_ref.get().to_dict() or {}
                motion_capture_enabled = settings_data.get('motion_capture_enabled', False)
                
                if motion_capture_paused_by_stream and motion_capture_enabled:
                     logger.info("Camera Server crashed. Re-activating Motion Capture.")
                     start_motion_capture()
                     motion_capture_paused_by_stream = False

            if config_updater_process and config_updater_process.poll() is not None:
                logger.error(f"[HEALTH CHECK] Config Updater (PID {config_updater_process.pid}) died unexpectedly. Resetting state.")
                stop_config_updater()

            if motion_capture_process and motion_capture_process.poll() is not None:
                logger.error(f"[HEALTH CHECK] Motion Capture (PID {motion_capture_process.pid}) died unexpectedly. Resetting state.")
                stop_motion_capture() # This sets current_motion_capture_enabled_state to False
